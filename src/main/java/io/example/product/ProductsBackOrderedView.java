...
@ViewId("products-back-ordered")
@Table("products_back_ordered")
@Subscribe.EventSourcedEntity(value = ProductEntity.class, ignoreUnknown = true)
public class ProductsBackOrderedView extends View<ProductEntity.State> {
  private static final Logger log = LoggerFactory.getLogger(ProductsView.class);

  @GetMapping("/products-back-ordered/{nextPageToken}")
  @Query("""
      ...
      OFFSET page_token_offset(:nextPageToken)
       LIMIT 100
      """)
  public Products getProductsBackOrdered(@PathVariable String nextPageToken) {
    return null;
  }
